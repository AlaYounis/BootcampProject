...
namespace HTUproject.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // var SkillAndCv = new SkillAndCv { UserId = user.Id, ApplicationUser = user.Id };
                    }
                    ViewBag.Roles = new SelectList(db.Roles.Where(a => !a.Name.Contains("Admins")).ToList(), "Name", "Name");
                    AddErrors(result);
                }
        ...
        {
            var dir = new System.IO.DirectoryInfo(Server.MapPath("~/UploadedCV"));
        ...
        [HttpPost]
        public ActionResult RegisterStudent(RegisterViewModel Account, string ImageName)
        {
            if (Request.Files.Count > 0)
            {
                Account.File = Request.Files[0];
            }
            ...
            if (!(Account.File.ContentType == "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ||
                Account.File.ContentType == "application/pdf"))
            {
            ...
            try
            {
                string fileName = Guid.NewGuid() + Path.GetExtension(Account.File.FileName);
                Account.File.SaveAs(Path.Combine(Server.MapPath("~/UploadedCV"), fileName));
                using (ApplicationDbContext db = new ApplicationDbContext())
                {
                    var user1 = User.Identity.GetUserId();
                    var user = db.Users.Where(x => x.Id == user1).SingleOrDefault();
        ...
        [HttpPost]
        public ActionResult RegisterTeacher(RegisterViewModel Account)
        {
